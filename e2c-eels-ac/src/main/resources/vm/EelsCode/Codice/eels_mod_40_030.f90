...
module eels
	...
	integer(ip),parameter	:: Nnode_x		= 201			!Numero di nodi
	integer(ip),parameter	:: Nnode_y		= 169
	integer(ip),parameter	:: Nnode_z		= 17
	!integer(ip),parameter	:: McCvert		= 10			!Vertici del Poligono di McCleave
	real(rp),parameter	:: length_x		= 100.5_rp		!Ampiezza del dominio [∞]
	real(rp),parameter	:: length_y		= 77.5_rp		![∞]
	real(rp),parameter	:: length_z		= 782.1_rp		![m]
...
!	real(rp),dimension(:),allocatable				:: invlambda
	...
	character(len=11),dimension(:),allocatable 		:: speedfiles 	!nomi dei file da caricare per le velocit‡
	character(len=11),dimension(:),allocatable 		:: tempfiles 	!nomi dei file da caricare per le temperature
	character(len=11),dimension(:),allocatable 		:: saltfiles 	!nomi dei file da caricare per la salinit‡
...
!	real(rp) :: delta_mortality
	...
	integer(ip) year_length				! anni simulati
...
!		enddo
		...
		allocate(speedfiles(24*time+4))
		allocate(tempfiles(24*time+4))
		allocate(saltfiles(24*time+4))
		allocate(g_month(24*time+4))
...
! coordinate di y e z da file (in x hanno delta costante)
		open(77,file="data/ocean/coordinate_y.dat",access='sequential')
		open(78,file="data/ocean/coordinate_z.dat",access='sequential')
		coord(1)=-100.25
		do h=1, Nnode_x
			do k=1, Nnode_y
				read(77,*) coord(2)
...
!del caricamento precedente)
	subroutine modOcean(first)
		...
		!character filename
		integer(ip) h,k,f,j,flag
		character(len=11) temp
		character(len=2) first_year, length_years
...
!	carico i nomi dei file da aprire solo per l'intervallo di durata della simulazione
			open(111,file="data/ocean/speed.dat", access='sequential')
			open(121,file="data/ocean/temp.dat", access='sequential')
			open(131,file="data/ocean/salt.dat", access='sequential')
			if (year_start/=58) then
				do h=1,((year_start-58)*12)
...
! scrivo un file con le durate in giorni di ciascun mese ed il numero del mese
			...
			!write(first_year,'(I2)') year_start
             !           write(length_years,'(I2)') year_length
!
 !                       command = "./transferFiles.sh "//first_year//" "//length_years//" "//ip_code
  !                      flag = -10
   !                     !f = 0
!
...
!	definisco l'intervallo per il prossimo aggiornamento
		...
		!command = "sshpass -p bonfire ssh -o StrictHostKeyChecking=no "//data_ip_code// &
                !        " ""sh -c 'cd /media/OceanographicData/OceanData/; ./transferFiles_multi.sh " &
		...
		open(111,file="/root/OceanData/"//speedfiles(update),access='sequential')
		open(121,file="/root/OceanData/"//tempfiles(update),access='sequential')
		open(131,file="/root/OceanData/"//saltfiles(update),access='sequential')
		update=update+1
		!write(666,*) "speedfiles(update) = ",speedfiles(update)
		open(112,file="/root/OceanData/"//speedfiles(update),access='sequential')
		update=update+1
		...
		if (first) then
			...
			open(113,file="/root/OceanData/"//speedfiles(update-4),&
				access='sequential')
			open(123,file="/root/OceanData/"//tempfiles(update-4),&
				access='sequential')
			open(133,file="/root/OceanData/"//saltfiles(update-4),&
				access='sequential')
			open(114,file="/root/OceanData/"//speedfiles(update-3),&
				access='sequential')
...
!	assegno i valori iniziali alle larve dal file eels.dat
		if ((year>=year_start).and.(month>=month_start).and.&
			(month<month_stop).and.(year<year_stop)) then
			
			if ((day==1).and.(month==month_start).and.(year==year_start)) then
...
!	se √® lunga pi√π	della lunghezza limite per il moto verticale		
			if (eels_data(j,2)>threshold_length) then
				vel_p=BL_s*eels_data(j,2)*0.1   ![cm/s]
				if (up_z==1) then
					mu=((dmu_night*eels_data(j,2)+mu_night_start)+&
						(dmu_day*eels_data(j,2)+mu_day_start))*0.5
					sigma=((dsigma_night*eels_data(j,2)+ &
