...
public class ApplicationProfileTest {
	...
	public void testPojo() {
		...
		resourceElem = new Resource();
		resourceElem2 = new Resource();

		resourceElem.setStorage("@BonFIREDebianSqueezev5");
		resourceElem2.setNetwork("@BonFIREWAN");

		resourceCompute2.add(resourceElem);
		...
		// Flow Element
		assertEquals(5, applicationProfileObject.getFlow().getSequence().size());
		...
		assertEquals("task2", applicationProfileObject.getFlow().getSequence()
				.get(1).getTask());
		assertEquals(2, applicationProfileObject.getFlow().getSequence().get(2)
				.getBranches().get(0).getBranch().size());
		assertEquals(0.3,
		...
		assertEquals(5, applicationProfileObject.getFlow().getSequence().get(4)
				.getLoop().size());
		...
		// Requirement element
		assertEquals(4, applicationProfileObject.getRequirement()
				.getConstraints().size());
		...
		assertEquals(2, applicationProfileObject.getRequirement()
				.getConstraints().get(3).getValues().size());
		...
		assertEquals("Experiment description", applicationProfileObject
				.getExperimentDescriptor().getDescription());
		...
		assertEquals(2, applicationProfileObject.getExperimentDescriptor()
				.getResourcesCompute().size());
		// Compute 1
		...
		// Data element
		assertEquals(2, applicationProfileObject.getData().getDataDependency()
				.size());
	...
	public void jsonToObject() throws Exception {
		...
		String applicationProfileJson = "{ \"applicationprofile\": {"
				+ "\"flow\": {"
				+ "          \"sequence\": ["
				+ "              {\"task\":\"task1\"},"
				+ "              {\"task\":\"task2\"},"
		...
		ApplicationProfile applicationProfileObject = mapper.readValue(
				applicationProfileJson, ApplicationProfile.class);

		System.out.println(mapper.writeValueAsString(applicationProfileObject));
	...
	public void objectToJson() throws Exception {
		...
		applicationProfile.setFlow(flow);
		applicationProfile.setRequirement(requirement);
		applicationProfile.setExperimentDescriptor(expDesc);
		applicationProfile.setData(data);
		applicationProfile.setAdaptation(adaptation);
		...
		String applicationProfileString = mapper
				.writeValueAsString(applicationProfile);

		System.out
				.println("applicationProfileJson:" + applicationProfileString);

		ApplicationProfile applicationProfileObject = mapper.readValue(
				applicationProfileString, ApplicationProfile.class);
