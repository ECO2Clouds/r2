...
public class CostTest {
	...
	public void pojo() {
		...
		assertEquals(10l, cost.getClock().longValue());
		assertEquals(1.0d, cost.getValue().doubleValue(), 0.0001);
		assertEquals("name", cost.getName());
		assertEquals("units", cost.getUnity());
	}
	
	@Test
	public void testXML() throws Exception {
		String costXML = "<Cost xmlns=\"http://accounting.eco2clouds.eu/doc/schemas/xml\">"
							+ "<value>94.0</value>"
		...
		Cost cost = (Cost) jaxbUnmarshaller.unmarshal(new StringReader(costXML));
		
		assertEquals(1381512236l, cost.getClock().longValue());
		assertEquals(94.0d, cost.getValue().doubleValue(), 0.0001);
		assertEquals("Co2 producted per kWh", cost.getName());
		assertEquals("g\\/kWh", cost.getUnity());
	}
