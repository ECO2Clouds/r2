...
public class CoalTest {
	...
	public void pojo() {
		...
		assertEquals(10l, gaz.getClock().longValue());
		assertEquals(1.0d, gaz.getValue().doubleValue(), 0.0001);
		assertEquals("name", gaz.getName());
		assertEquals("units", gaz.getUnity());
	}
	
	@Test
	public void testXML() throws Exception {
		String coalXML = "<Gaz xmlns=\"http://accounting.eco2clouds.eu/doc/schemas/xml\">"
							+ "<value>94.0</value>"
		...
		JAXBContext jaxbContext = JAXBContext.newInstance(Gaz.class);
		Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
		Gaz gaz = (Gaz) jaxbUnmarshaller.unmarshal(new StringReader(coalXML));
		
		assertEquals(1381512236l, gaz.getClock().longValue());
		assertEquals(94.0d, gaz.getValue().doubleValue(), 0.0001);
		assertEquals("Co2 producted per kWh", gaz.getName());
		assertEquals("g\\/kWh", gaz.getUnity());
	}
	...
	public void objectToXml() throws Exception {
		...
		XPath xpath = XPath.newInstance("//bnf:Gaz");
