 ...
 */
public class SwapFreeTest {

	@Test 
	public void pojo() {
		SwapFree swapfree = new SwapFree();
		swapfree.setClock(10l);
		swapfree.setName("name");
		swapfree.setUnity("units");
		swapfree.setValue(1.0d);
		
		assertEquals(10l, swapfree.getClock().longValue());
		assertEquals(1.0d, swapfree.getValue().doubleValue(), 0.0001);
		assertEquals("name", swapfree.getName());
		assertEquals("units", swapfree.getUnity());
	}
	
	@Test
	public void testXML() throws Exception {
		String swapFreeXML = "<swapfree xmlns=\"http://accounting.eco2clouds.eu/doc/schemas/xml\">"
							+ "<value>94.0</value>"
		...
		JAXBContext jaxbContext = JAXBContext.newInstance(SwapFree.class);
		Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
		SwapFree swapfree = (SwapFree) jaxbUnmarshaller.unmarshal(new StringReader(swapFreeXML));
		
		assertEquals(1381512236l, swapfree.getClock().longValue());
		assertEquals(94.0d, swapfree.getValue().doubleValue(), 0.0001);
		assertEquals("Co2 producted per kWh", swapfree.getName());
		assertEquals("g\\/kWh", swapfree.getUnity());
	}
	...
	public void objectToXml() throws Exception {
		...
		marshaller.marshal(swapfree, out);
		...
		XPath xpath = XPath.newInstance("//bnf:swapfree");
