...
public class Co2RawTest {
	...
	public void pojo() {
		...
		co2Raw.setClock(10l);
		co2Raw.setName("name");
		co2Raw.setUnity("units");
		co2Raw.setValue(1.0d);
		
		assertEquals(10l, co2Raw.getClock().longValue());
		assertEquals(1.0d, co2Raw.getValue().doubleValue(), 0.0001);
		assertEquals("name", co2Raw.getName());
		assertEquals("units", co2Raw.getUnity());
	}
	
	@Test
	public void testXML() throws Exception {
		String co2XML = "<Co2Raw xmlns=\"http://accounting.eco2clouds.eu/doc/schemas/xml\">"
							+ "<value>94.0</value>"
		...
		JAXBContext jaxbContext = JAXBContext.newInstance(Co2Raw.class);
		Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
		Co2Raw co2Raw = (Co2Raw) jaxbUnmarshaller.unmarshal(new StringReader(co2XML));
		
		assertEquals(1381512236l, co2Raw.getClock().longValue());
		assertEquals(94.0d, co2Raw.getValue().doubleValue(), 0.0001);
		assertEquals("Co2 producted per kWh", co2Raw.getName());
		assertEquals("g\\/kWh", co2Raw.getUnity());
	}
	...
	public void objectToXml() throws Exception {
		...
		XPath xpath = XPath.newInstance("//bnf:Co2Raw");
