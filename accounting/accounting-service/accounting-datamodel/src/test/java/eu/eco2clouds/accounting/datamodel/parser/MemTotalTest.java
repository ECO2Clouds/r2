 ...
 */
public class MemTotalTest {

	@Test 
	public void pojo() {
		MemTotal memtotal = new MemTotal();
		memtotal.setClock(10l);
		memtotal.setName("name");
		memtotal.setUnity("units");
		memtotal.setValue(1.0d);
		
		assertEquals(10l, memtotal.getClock().longValue());
		assertEquals(1.0d, memtotal.getValue().doubleValue(), 0.0001);
		assertEquals("name", memtotal.getName());
		assertEquals("units", memtotal.getUnity());
	}
	
	@Test
	public void testXML() throws Exception {
		String memtotalXML = "<memtotal xmlns=\"http://accounting.eco2clouds.eu/doc/schemas/xml\">"
							+ "<value>94.0</value>"
		...
		JAXBContext jaxbContext = JAXBContext.newInstance(MemTotal.class);
		Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
		MemTotal memtotal = (MemTotal) jaxbUnmarshaller.unmarshal(new StringReader(memtotalXML));
		
		assertEquals(1381512236l, memtotal.getClock().longValue());
		assertEquals(94.0d, memtotal.getValue().doubleValue(), 0.0001);
		assertEquals("Co2 producted per kWh", memtotal.getName());
		assertEquals("g\\/kWh", memtotal.getUnity());
	}
	...
	public void objectToXml() throws Exception {
		...
		marshaller.marshal(memtotal, out);
		...
		XPath xpath = XPath.newInstance("//bnf:memtotal");
