 ...
 */
public class TotalCo2Test {

	@Test 
	public void pojo() {
		TotalCo2 total = new TotalCo2();
		total.setClock(10l);
		total.setName("name");
		total.setUnity("units");
		total.setValue(1.0d);
		
		assertEquals(10l, total.getClock().longValue());
		assertEquals(1.0d, total.getValue().doubleValue(), 0.0001);
		assertEquals("name", total.getName());
		assertEquals("units", total.getUnity());
	}
	
	@Test
	public void testXML() throws Exception {
		String totalXML = "<total_co2 xmlns=\"http://accounting.eco2clouds.eu/doc/schemas/xml\">"
							+ "<value>94.0</value>"
		...
		TotalCo2 total = (TotalCo2) jaxbUnmarshaller.unmarshal(new StringReader(totalXML));
		
		assertEquals(1381512236l, total.getClock().longValue());
		assertEquals(94.0d, total.getValue().doubleValue(), 0.0001);
		assertEquals("Co2 producted per kWh", total.getName());
		assertEquals("g\\/kWh", total.getUnity());
	}
	...
	public void objectToXml() throws Exception {
		...
		XPath xpath = XPath.newInstance("//bnf:total_co2");
