...
public class PDUFrTest {
	...
	public void pojo() {
		...
		pDUFr.setClock(10l);
		pDUFr.setName("name");
		pDUFr.setUnity("units");
		pDUFr.setValue(1.0d);
		
		assertEquals(10l, pDUFr.getClock().longValue());
		assertEquals(1.0d, pDUFr.getValue().doubleValue(), 0.0001);
		assertEquals("name", pDUFr.getName());
		assertEquals("units", pDUFr.getUnity());
	}
	
	@Test
	public void testXML() throws Exception {
		String pDUFrXML = "<PDU_fr xmlns=\"http://accounting.eco2clouds.eu/doc/schemas/xml\">"
							+ "<value>94.0</value>"
		...
		JAXBContext jaxbContext = JAXBContext.newInstance(PDUFr.class);
		Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
		PDUFr pDUFr = (PDUFr) jaxbUnmarshaller.unmarshal(new StringReader(pDUFrXML));
		
		assertEquals(1381512236l, pDUFr.getClock().longValue());
		assertEquals(94.0d, pDUFr.getValue().doubleValue(), 0.0001);
		assertEquals("Co2 producted per kWh", pDUFr.getName());
		assertEquals("g\\/kWh", pDUFr.getUnity());
	}
	...
	public void objectToXml() throws Exception {
		...
		XPath xpath = XPath.newInstance("//bnf:PDU_fr");
