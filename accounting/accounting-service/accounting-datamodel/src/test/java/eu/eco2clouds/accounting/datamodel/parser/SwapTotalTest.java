 ...
 */
public class SwapTotalTest {

	@Test 
	public void pojo() {
		SwapTotal swaptotal = new SwapTotal();
		swaptotal.setClock(10l);
		swaptotal.setName("name");
		swaptotal.setUnity("units");
		swaptotal.setValue(1.0d);
		
		assertEquals(10l, swaptotal.getClock().longValue());
		assertEquals(1.0d, swaptotal.getValue().doubleValue(), 0.0001);
		assertEquals("name", swaptotal.getName());
		assertEquals("units", swaptotal.getUnity());
	}
	
	@Test
	public void testXML() throws Exception {
		String swapTotalXML = "<swaptotal xmlns=\"http://accounting.eco2clouds.eu/doc/schemas/xml\">"
							+ "<value>94.0</value>"
		...
		JAXBContext jaxbContext = JAXBContext.newInstance(SwapTotal.class);
		Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
		SwapTotal swaptotal = (SwapTotal) jaxbUnmarshaller.unmarshal(new StringReader(swapTotalXML));
		
		assertEquals(1381512236l, swaptotal.getClock().longValue());
		assertEquals(94.0d, swaptotal.getValue().doubleValue(), 0.0001);
		assertEquals("Co2 producted per kWh", swaptotal.getName());
		assertEquals("g\\/kWh", swaptotal.getUnity());
	}
	...
	public void objectToXml() throws Exception {
		...
		marshaller.marshal(swaptotal, out);
		...
		XPath xpath = XPath.newInstance("//bnf:swaptotal");
