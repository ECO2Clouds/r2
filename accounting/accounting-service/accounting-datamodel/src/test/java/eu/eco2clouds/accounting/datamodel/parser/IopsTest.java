...
public class IopsTest {
	...
	public void pojo() {
		...
		assertEquals(10l, iops.getClock().longValue());
		assertEquals(1.0d, iops.getValue().doubleValue(), 0.0001);
		assertEquals("name", iops.getName());
		assertEquals("units", iops.getUnity());
	}
	
	@Test
	public void testXML() throws Exception {
		String iopsXML = "<iops xmlns=\"http://accounting.eco2clouds.eu/doc/schemas/xml\">"
							+ "<value>94.0</value>"
		...
		JAXBContext jaxbContext = JAXBContext.newInstance(Iops.class);
		Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
		Iops iops = (Iops) jaxbUnmarshaller.unmarshal(new StringReader(iopsXML));
		
		assertEquals(1381512236l, iops.getClock().longValue());
		assertEquals(94.0d, iops.getValue().doubleValue(), 0.0001);
		assertEquals("Co2 producted per kWh", iops.getName());
		assertEquals("g\\/kWh", iops.getUnity());
	}
	...
	public void objectToXml() throws Exception {
		...
		XPath xpath = XPath.newInstance("//bnf:iops");
