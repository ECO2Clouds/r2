...
public class CPULoadTest {
	...
	public void pojo() {
		...
		cpuload.setClock(10l);
		cpuload.setName("name");
		cpuload.setUnity("units");
		cpuload.setValue(1.0d);
		
		assertEquals(10l, cpuload.getClock().longValue());
		assertEquals(1.0d, cpuload.getValue().doubleValue(), 0.0001);
		assertEquals("name", cpuload.getName());
		assertEquals("units", cpuload.getUnity());
	}
	
	@Test
	public void testXML() throws Exception {
		String oilXML = "<cpuload xmlns=\"http://accounting.eco2clouds.eu/doc/schemas/xml\">"
							+ "<value>94.0</value>"
		...
		JAXBContext jaxbContext = JAXBContext.newInstance(CPULoad.class);
		Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
		CPULoad cpuload = (CPULoad) jaxbUnmarshaller.unmarshal(new StringReader(oilXML));
		
		assertEquals(1381512236l, cpuload.getClock().longValue());
		assertEquals(94.0d, cpuload.getValue().doubleValue(), 0.0001);
		assertEquals("Co2 producted per kWh", cpuload.getName());
		assertEquals("g\\/kWh", cpuload.getUnity());
	}
	...
	public void objectToXml() throws Exception {
		...
		marshaller.marshal(cpuload, out);
		...
		XPath xpath = XPath.newInstance("//bnf:cpuload");
