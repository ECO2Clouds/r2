 ...
 */
public class DiskTotalTest {

	@Test 
	public void pojo() {
		DiskTotal disktotal = new DiskTotal();
		disktotal.setClock(10l);
		disktotal.setName("name");
		disktotal.setUnity("units");
		disktotal.setValue(1.0d);
		
		assertEquals(10l, disktotal.getClock().longValue());
		assertEquals(1.0d, disktotal.getValue().doubleValue(), 0.0001);
		assertEquals("name", disktotal.getName());
		assertEquals("units", disktotal.getUnity());
	}
	
	@Test
	public void testXML() throws Exception {
		String diskTotalXML = "<disktotal xmlns=\"http://accounting.eco2clouds.eu/doc/schemas/xml\">"
							+ "<value>94.0</value>"
		...
		JAXBContext jaxbContext = JAXBContext.newInstance(DiskTotal.class);
		Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
		DiskTotal disktotal = (DiskTotal) jaxbUnmarshaller.unmarshal(new StringReader(diskTotalXML));
		
		assertEquals(1381512236l, disktotal.getClock().longValue());
		assertEquals(94.0d, disktotal.getValue().doubleValue(), 0.0001);
		assertEquals("Co2 producted per kWh", disktotal.getName());
		assertEquals("g\\/kWh", disktotal.getUnity());
	}
	...
	public void objectToXml() throws Exception {
		...
		marshaller.marshal(disktotal, out);
		...
		XPath xpath = XPath.newInstance("//bnf:disktotal");
