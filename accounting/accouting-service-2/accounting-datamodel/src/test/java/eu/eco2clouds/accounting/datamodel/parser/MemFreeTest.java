 ...
 */
public class MemFreeTest {

	@Test 
	public void pojo() {
		MemFree memfree = new MemFree();
		memfree.setClock(10l);
		memfree.setName("name");
		memfree.setUnity("units");
		memfree.setValue(1.0d);
		
		assertEquals(10l, memfree.getClock().longValue());
		assertEquals(1.0d, memfree.getValue().doubleValue(), 0.0001);
		assertEquals("name", memfree.getName());
		assertEquals("units", memfree.getUnity());
	}
	
	@Test
	public void testXML() throws Exception {
		String memfreeXML = "<memfree xmlns=\"http://accounting.eco2clouds.eu/doc/schemas/xml\">"
							+ "<value>94.0</value>"
		...
		JAXBContext jaxbContext = JAXBContext.newInstance(MemFree.class);
		Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
		MemFree memfree = (MemFree) jaxbUnmarshaller.unmarshal(new StringReader(memfreeXML));
		
		assertEquals(1381512236l, memfree.getClock().longValue());
		assertEquals(94.0d, memfree.getValue().doubleValue(), 0.0001);
		assertEquals("Co2 producted per kWh", memfree.getName());
		assertEquals("g\\/kWh", memfree.getUnity());
	}
	...
	public void objectToXml() throws Exception {
		...
		marshaller.marshal(memfree, out);
		...
		XPath xpath = XPath.newInstance("//bnf:memfree");
