 ...
 */
public class MemUsedTest {

	@Test 
	public void pojo() {
		MemUsed memused = new MemUsed();
		memused.setClock(10l);
		memused.setName("name");
		memused.setUnity("units");
		memused.setValue(1.0d);
		
		assertEquals(10l, memused.getClock().longValue());
		assertEquals(1.0d, memused.getValue().doubleValue(), 0.0001);
		assertEquals("name", memused.getName());
		assertEquals("units", memused.getUnity());
	}
	
	@Test
	public void testXML() throws Exception {
		String memusedXML = "<memused xmlns=\"http://accounting.eco2clouds.eu/doc/schemas/xml\">"
							+ "<value>94.0</value>"
		...
		JAXBContext jaxbContext = JAXBContext.newInstance(MemUsed.class);
		Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
		MemUsed memused = (MemUsed) jaxbUnmarshaller.unmarshal(new StringReader(memusedXML));
		
		assertEquals(1381512236l, memused.getClock().longValue());
		assertEquals(94.0d, memused.getValue().doubleValue(), 0.0001);
		assertEquals("Co2 producted per kWh", memused.getName());
		assertEquals("g\\/kWh", memused.getUnity());
	}
	...
	public void objectToXml() throws Exception {
		...
		marshaller.marshal(memused, out);
		...
		XPath xpath = XPath.newInstance("//bnf:memused");
