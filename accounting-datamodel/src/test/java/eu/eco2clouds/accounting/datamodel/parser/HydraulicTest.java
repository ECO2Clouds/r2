...
public class HydraulicTest {
	...
	public void pojo() {
		...
		hydraulic.setClock(10l);
		hydraulic.setName("name");
		hydraulic.setUnity("units");
		hydraulic.setValue(1.0d);
		
		assertEquals(10l, hydraulic.getClock().longValue());
		assertEquals(1.0d, hydraulic.getValue().doubleValue(), 0.0001);
		assertEquals("name", hydraulic.getName());
		assertEquals("units", hydraulic.getUnity());
	}
	
	@Test
	public void testXML() throws Exception {
		String hydraulicXML = "<Hydraulic xmlns=\"http://accounting.eco2clouds.eu/doc/schemas/xml\">"
							+ "<value>94.0</value>"
		...
		JAXBContext jaxbContext = JAXBContext.newInstance(Hydraulic.class);
		Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
		Hydraulic hydraulic = (Hydraulic) jaxbUnmarshaller.unmarshal(new StringReader(hydraulicXML));
		
		assertEquals(1381512236l, hydraulic.getClock().longValue());
		assertEquals(94.0d, hydraulic.getValue().doubleValue(), 0.0001);
		assertEquals("Co2 producted per kWh", hydraulic.getName());
		assertEquals("g\\/kWh", hydraulic.getUnity());
	}
	...
	public void objectToXml() throws Exception {
		...
		marshaller.marshal(hydraulic, out);
		...
		XPath xpath = XPath.newInstance("//bnf:Hydraulic");
