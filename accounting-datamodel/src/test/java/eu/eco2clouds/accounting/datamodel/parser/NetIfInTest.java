 ...
 */
public class NetIfInTest {

	@Test 
	public void pojo() {
		NetIfIn netifin = new NetIfIn();
		netifin.setClock(10l);
		netifin.setName("name");
		netifin.setUnity("units");
		netifin.setValue(1.0d);
		
		assertEquals(10l, netifin.getClock().longValue());
		assertEquals(1.0d, netifin.getValue().doubleValue(), 0.0001);
		assertEquals("name", netifin.getName());
		assertEquals("units", netifin.getUnity());
	}
	
	@Test
	public void testXML() throws Exception {
		String netifinXML = "<netifin xmlns=\"http://accounting.eco2clouds.eu/doc/schemas/xml\">"
							+ "<value>94.0</value>"
		...
		JAXBContext jaxbContext = JAXBContext.newInstance(NetIfIn.class);
		Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
		NetIfIn netifin = (NetIfIn) jaxbUnmarshaller.unmarshal(new StringReader(netifinXML));
		
		assertEquals(1381512236l, netifin.getClock().longValue());
		assertEquals(94.0d, netifin.getValue().doubleValue(), 0.0001);
		assertEquals("Co2 producted per kWh", netifin.getName());
		assertEquals("g\\/kWh", netifin.getUnity());
	}
	...
	public void objectToXml() throws Exception {
		...
		marshaller.marshal(netifin, out);
		...
		XPath xpath = XPath.newInstance("//bnf:netifin");
