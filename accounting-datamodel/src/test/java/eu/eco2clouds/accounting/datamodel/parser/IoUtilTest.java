...
public class IoUtilTest {
	...
	public void pojo() {
		...
		ioUtil.setClock(10l);
		ioUtil.setName("name");
		ioUtil.setUnity("units");
		ioUtil.setValue(1.0d);
		
		assertEquals(10l, ioUtil.getClock().longValue());
		assertEquals(1.0d, ioUtil.getValue().doubleValue(), 0.0001);
		assertEquals("name", ioUtil.getName());
		assertEquals("units", ioUtil.getUnity());
	}
	
	@Test
	public void testXML() throws Exception {
		String ioUtilXML = "<ioutil xmlns=\"http://accounting.eco2clouds.eu/doc/schemas/xml\">"
							+ "<value>94.0</value>"
		...
		JAXBContext jaxbContext = JAXBContext.newInstance(IoUtil.class);
		Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
		IoUtil ioUtil = (IoUtil) jaxbUnmarshaller.unmarshal(new StringReader(ioUtilXML));
		
		assertEquals(1381512236l, ioUtil.getClock().longValue());
		assertEquals(94.0d, ioUtil.getValue().doubleValue(), 0.0001);
		assertEquals("Co2 producted per kWh", ioUtil.getName());
		assertEquals("g\\/kWh", ioUtil.getUnity());
	}
	...
	public void objectToXml() throws Exception {
		...
		XPath xpath = XPath.newInstance("//bnf:ioutil");
