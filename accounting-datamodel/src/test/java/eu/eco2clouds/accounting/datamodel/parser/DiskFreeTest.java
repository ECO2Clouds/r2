...
public class DiskFreeTest {
	...
	public void pojo() {
		...
		diskfree.setClock(10l);
		diskfree.setName("name");
		diskfree.setUnity("units");
		diskfree.setValue(1.0d);
		
		assertEquals(10l, diskfree.getClock().longValue());
		assertEquals(1.0d, diskfree.getValue().doubleValue(), 0.0001);
		assertEquals("name", diskfree.getName());
		assertEquals("units", diskfree.getUnity());
	}
	
	@Test
	public void testXML() throws Exception {
		String diskFreeXML = "<diskfree xmlns=\"http://accounting.eco2clouds.eu/doc/schemas/xml\">"
							+ "<value>94.0</value>"
		...
		JAXBContext jaxbContext = JAXBContext.newInstance(DiskFree.class);
		Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
		DiskFree diskfree = (DiskFree) jaxbUnmarshaller.unmarshal(new StringReader(diskFreeXML));
		
		assertEquals(1381512236l, diskfree.getClock().longValue());
		assertEquals(94.0d, diskfree.getValue().doubleValue(), 0.0001);
		assertEquals("Co2 producted per kWh", diskfree.getName());
		assertEquals("g\\/kWh", diskfree.getUnity());
	}
	...
	public void objectToXml() throws Exception {
		...
		marshaller.marshal(diskfree, out);
		...
		XPath xpath = XPath.newInstance("//bnf:diskfree");
