 ...
 */
public class NetIfOutTest {

	@Test 
	public void pojo() {
		NetIfOut netifout = new NetIfOut();
		netifout.setClock(10l);
		netifout.setName("name");
		netifout.setUnity("units");
		netifout.setValue(1.0d);
		
		assertEquals(10l, netifout.getClock().longValue());
		assertEquals(1.0d, netifout.getValue().doubleValue(), 0.0001);
		assertEquals("name", netifout.getName());
		assertEquals("units", netifout.getUnity());
	}
	
	@Test
	public void testXML() throws Exception {
		String netifoutXML = "<netifout xmlns=\"http://accounting.eco2clouds.eu/doc/schemas/xml\">"
							+ "<value>94.0</value>"
		...
		JAXBContext jaxbContext = JAXBContext.newInstance(NetIfOut.class);
		Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
		NetIfOut netifout = (NetIfOut) jaxbUnmarshaller.unmarshal(new StringReader(netifoutXML));
		
		assertEquals(1381512236l, netifout.getClock().longValue());
		assertEquals(94.0d, netifout.getValue().doubleValue(), 0.0001);
		assertEquals("Co2 producted per kWh", netifout.getName());
		assertEquals("g\\/kWh", netifout.getUnity());
	}
	...
	public void objectToXml() throws Exception {
		...
		marshaller.marshal(netifout, out);
		...
		XPath xpath = XPath.newInstance("//bnf:netifout");
