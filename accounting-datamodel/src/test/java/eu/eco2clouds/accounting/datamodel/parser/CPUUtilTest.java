...
public class CPUUtilTest {
	...
	public void pojo() {
		...
		cpuutil.setClock(10l);
		cpuutil.setName("name");
		cpuutil.setUnity("units");
		cpuutil.setValue(1.0d);
		
		assertEquals(10l, cpuutil.getClock().longValue());
		assertEquals(1.0d, cpuutil.getValue().doubleValue(), 0.0001);
		assertEquals("name", cpuutil.getName());
		assertEquals("units", cpuutil.getUnity());
	}
	
	@Test
	public void testXML() throws Exception {
		String cpuUtilXML = "<cpuutil xmlns=\"http://accounting.eco2clouds.eu/doc/schemas/xml\">"
							+ "<value>94.0</value>"
		...
		JAXBContext jaxbContext = JAXBContext.newInstance(CPUUtil.class);
		Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
		CPUUtil cpuutil = (CPUUtil) jaxbUnmarshaller.unmarshal(new StringReader(cpuUtilXML));
		
		assertEquals(1381512236l, cpuutil.getClock().longValue());
		assertEquals(94.0d, cpuutil.getValue().doubleValue(), 0.0001);
		assertEquals("Co2 producted per kWh", cpuutil.getName());
		assertEquals("g\\/kWh", cpuutil.getUnity());
	}
	...
	public void objectToXml() throws Exception {
		...
		marshaller.marshal(cpuutil, out);
		...
		XPath xpath = XPath.newInstance("//bnf:cpuutil");
