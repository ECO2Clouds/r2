...
public class DiskUsageTest {
	...
	public void pojo() {
		...
		diskusage.setClock(10l);
		diskusage.setName("name");
		diskusage.setUnity("units");
		diskusage.setValue(1.0d);
		
		assertEquals(10l, diskusage.getClock().longValue());
		assertEquals(1.0d, diskusage.getValue().doubleValue(), 0.0001);
		assertEquals("name", diskusage.getName());
		assertEquals("units", diskusage.getUnity());
	}
	
	@Test
	public void testXML() throws Exception {
		String diskUsageXML = "<diskusage xmlns=\"http://accounting.eco2clouds.eu/doc/schemas/xml\">"
							+ "<value>94.0</value>"
		...
		JAXBContext jaxbContext = JAXBContext.newInstance(DiskUsage.class);
		Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
		DiskUsage diskusage = (DiskUsage) jaxbUnmarshaller.unmarshal(new StringReader(diskUsageXML));
		
		assertEquals(1381512236l, diskusage.getClock().longValue());
		assertEquals(94.0d, diskusage.getValue().doubleValue(), 0.0001);
		assertEquals("Co2 producted per kWh", diskusage.getName());
		assertEquals("g\\/kWh", diskusage.getUnity());
	}
	...
	public void objectToXml() throws Exception {
		...
		marshaller.marshal(diskusage, out);
		...
		XPath xpath = XPath.newInstance("//bnf:diskusage");
