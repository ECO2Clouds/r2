...
public class DiskReadWriteTest {
	...
	public void pojo() {
		...
		diskReadWrite.setClock(10l);
		diskReadWrite.setName("name");
		diskReadWrite.setUnity("units");
		diskReadWrite.setValue(1.0d);
		
		assertEquals(10l, diskReadWrite.getClock().longValue());
		assertEquals(1.0d, diskReadWrite.getValue().doubleValue(), 0.0001);
		assertEquals("name", diskReadWrite.getName());
		assertEquals("units", diskReadWrite.getUnity());
	}
	
	@Test
	public void testXML() throws Exception {
		String co2XML = "<disk_read_write xmlns=\"http://accounting.eco2clouds.eu/doc/schemas/xml\">"
							+ "<value>94.0</value>"
		...
		JAXBContext jaxbContext = JAXBContext.newInstance(DiskReadWrite.class);
		Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
		DiskReadWrite co2 = (DiskReadWrite) jaxbUnmarshaller.unmarshal(new StringReader(co2XML));
	...
	public void objectToXml() throws Exception {
		DiskReadWrite co2 = new DiskReadWrite();
		co2.setClock(10l);
		co2.setName("name");
		co2.setUnity("units");
		co2.setValue(1.0d);
		...
		XPath xpath = XPath.newInstance("//bnf:disk_read_write");
