...
subroutine sim(step,stepup,counter_larvae)
	...
	integer(ip) j,i,h,k,a,b,c,f , BS_x, BS_y
...
!	valuto la sopravvivenza	
						...
						if (((survival>sigma).or.(eels_data(j,1)>=time_killer)).or.(eels_int(j,1)==-1)) then
